...
int main(void)
{
    ...
    // Create threads
    const uint32_t THREAD_STACK_SIZE_WORDS_IDLE      = 64;
    const uint32_t THREAD_STACK_SIZE_WORDS_THREAD1   = 64;
    const uint32_t THREAD_STACK_SIZE_WORDS_THREAD2   = 64;
    const uint32_t THREAD_STACK_SIZE_WORDS_THREAD3   = 64;
    uint32_t stack0[THREAD_STACK_SIZE_WORDS_IDLE]    = {0xDEADBEEF};
    uint32_t stack1[THREAD_STACK_SIZE_WORDS_THREAD1] = {0xDEADBEEF};
    uint32_t stack2[THREAD_STACK_SIZE_WORDS_THREAD2] = {0xDEADBEEF};
    uint32_t stack3[THREAD_STACK_SIZE_WORDS_THREAD3] = {0xDEADBEEF};
    ...
    // Register Threads
    OS::Error_t status = OS::RegisterThread(th0, THREAD_STACK_SIZE_WORDS_IDLE);
    PrintOsError(status, (__LINE__ - 1));
    status = OS::RegisterThread(th1, THREAD_STACK_SIZE_WORDS_THREAD1);
    PrintOsError(status, (__LINE__ - 1));
    status = OS::RegisterThread(th2, THREAD_STACK_SIZE_WORDS_THREAD2);
    PrintOsError(status, (__LINE__ - 1));
    status = OS::RegisterThread(th3, THREAD_STACK_SIZE_WORDS_THREAD3);
    PrintOsError(status, (__LINE__ - 1));

    // Start OS; never returns
    OS::Start(&stack0[THREAD_STACK_SIZE_WORDS_IDLE - 1]);
}
